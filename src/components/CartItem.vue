...
<template>
  <v-list-item>
    ...
    <v-list-item-title>
      <router-link :to="`/${item.id}`" class="text-decoration-none text-black">{{ item.name }}</router-link>
    </v-list-item-title>
    ...
    <div class="d-flex ga-4 mt-1 align-end">
      <div class="text-subtitle-2 d-inline-flex align-center border border-opacity-50 border-primary rounded-pill">
        <v-btn @click.stop="handleDecrease" variant="text"
          :icon="item.quantity > 1 ? 'mdi-minus' : 'mdi-trash-can-outline'" size="small" />
        {{ item.quantity }}
        <v-btn @click.stop="() => emits('increase', item)" variant="text" icon="mdi-plus" size="small" />
      </div>

      <v-btn @click.stop="() => emits('delete', item.id)" variant="plain" size="small" slim :ripple="false"
